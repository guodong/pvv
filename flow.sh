...
docker exec s3 ifconfig i1 10.0.2.6/24
...
# set flow rules
docker exec s1 ovs-ofctl add-flow s in_port=1,actions=output:4
docker exec s1 ovs-ofctl add-flow s in_port=4,actions=output:1
docker exec s1 ovs-ofctl add-flow s in_port=2,actions=output:5
docker exec s1 ovs-ofctl add-flow s in_port=5,actions=output:2
docker exec s1 ovs-ofctl add-flow s in_port=3,actions=output:6
docker exec s1 ovs-ofctl add-flow s in_port=6,actions=output:3

docker exec s2 ovs-ofctl add-flow s in_port=1,actions=output:3
docker exec s2 ovs-ofctl add-flow s in_port=1,actions=output:1
docker exec s2 ovs-ofctl add-flow s in_port=2,actions=output:4
docker exec s2 ovs-ofctl add-flow s in_port=4,actions=output:2

docker exec s3 ovs-ofctl add-flow s in_port=1,actions=output:4
docker exec s3 ovs-ofctl add-flow s in_port=4,actions=output:1
docker exec s3 ovs-ofctl add-flow s in_port=2,actions=output:5
docker exec s3 ovs-ofctl add-flow s in_port=5,actions=output:2
docker exec s3 ovs-ofctl add-flow s in_port=3,actions=output:6
